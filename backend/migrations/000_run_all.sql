...
-- originales (047-052). Patrón: SELECT abierto a autenticados + escritura
-- restringida a admin_rrhh. El aislamiento multiempresa lo maneja el backend
-- (header X-Empresa-Id), no RLS — ninguna policy filtra por empresa_id.
-- Idempotente: cada CREATE POLICY se protege con guard sobre pg_policies.
