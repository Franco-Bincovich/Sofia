...
import type { FilaPreview, ImportacionPreview, ImportacionResult } from "@/types/importacion"
...
export async function previewImportacionCSV(file: File): Promise<ImportacionPreview> {
...
export async function confirmarImportacion(filas: FilaPreview[]): Promise<ImportacionResult> {
  const session = getSession()
